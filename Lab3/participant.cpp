...
void participant::connect_coordinator()
{
    ...
    addr_sera.sin_port = htons(coordinator_info.port);
    addr_sera.sin_addr.s_addr = inet_addr(coordinator_info.ip.c_str());
    while(connect(clientfd, (struct sockaddr*)&addr_sera, sizeof(addr_sera)) != 0) {
        cout << "connect coordinator error" << endl;
    }
    close(clientfd);
}
//构造函数，从配置文件中读取内容并初始化相应变量
participant::participant(char * fln)
{
    backlog = 20;
    fstream input(fln);
    string str;
