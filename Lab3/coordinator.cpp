...
coordinator::coordinator(char *fln)
{
    fstream input(fln);
    string str;
    // epoll侦听的句柄数
    ...
    // input.eof() 是一个成员函数，用于判断文件流对象 input 的当前位置是否已经到达文件末尾
    // 解析协同者配置文件，协同者IP、端口，参与者IP、端口
    while(!input.eof()) 
    {
        getline(input, str);
...
{
    set<int>::iterator it;
    struct sockaddr_in addr_sera;
    // 3：全部  5：启动时尝试建立连接 -> 尝试与所有坏的参与者建立连接
    if (mode == 3 || mode == 5) {
        vector<thread> t;
        for(it = failed_participant.begin();it != failed_participant.end();) {
            ...
            memset(&addr_sera, 0, sizeof(addr_sera));
            addr_sera.sin_family = AF_INET;
            addr_sera.sin_port = htons(participant_info[i].port);
            addr_sera.sin_addr.s_addr = inet_addr(participant_info[i].ip.c_str());
            if ((connect(clientfd, (struct sockaddr*)&addr_sera, sizeof(addr_sera))) != 0)
            {
                cout << "connect" << clientfd << "error1 !" << endl;
                if(mode == 5)
                    t.push_back(move(thread(try_connect,this, i, clientfd, addr_sera)));// 创建线程尝试连接
                else 
                    close(clientfd);
                it ++;
                continue;
...
void coordinator::try_connect(coordinator* cor,int i, int clientfd, struct sockaddr_in addr_sera)
{
    int n = 20;
    while (n--)
    {
