...
struct Dance
{
    ...
    void get_answer(char *ch, int* myBoard)
    {
        for(int i = 0; i < N; i ++)
            ch[i] = myBoard[i] + '0';
    }
...
void input_(char* problem, int* myBoard, int tmp)
{
    for(int cell = 0; cell < N; ++cell)
    {
        myBoard[cell] = problem[cell] - '0';
        if(!(0 <= myBoard[cell] && myBoard[cell] <= NUM))
        {
            printf("wrong : myBoard[%d] = %d in problem[%d]\n", cell, myBoard[cell], tmp);
            assert(0 <= myBoard[cell] && myBoard[cell] <= NUM);
        }
    }
}
...
{
    input_(ch_, myBoard, tmp);
    Dance d(myBoard);
    d.solve();
    d.get_answer(ch_, myBoard);
}
